...
async function getTireTypeIds() {
  const response = await fetch(`${STRAPI_URL}/api/loai-lops`);
  if (!response.ok) {
  ...
  if (!Array.isArray(data)) {
    throw new Error('API response for Tire Types is not in the expected array format.');
  }
  ...
  data.forEach(type => {
    ...
    if (type && type.name && type.id) {
      ...
      console.warn('Warning: Found a type entry with a missing name or id:', type);
    }
  });
  console.log('Fetched Tire Type IDs:', typeMap);
  return typeMap;
}

async function seed() {
  if (!STRAPI_API_TOKEN) {
    console.error('Error: STRAPI_API_TOKEN is not set. Please provide the token.');
    process.exit(1);
  }

  console.log('Starting seed process...');

  const typeIds = await getTireTypeIds();

  for (const product of treadPatterns) {
    const typeId = typeIds[product.type];
    if (!typeId) {
      console.warn(`Warning: Could not find ID for type "${product.type}". Skipping product "${product.name}".`);
      continue;
    }
    ...
    try {
      ...
      const response = await fetch(`${STRAPI_URL}/api/san-phams`, {
        method: 'POST',
        headers: {
      ...
      console.error(`An error occurred while creating ${product.name}:`, error);
    }
  }

  console.log('\nSeed process finished. Go to your Strapi admin panel to see the new products. Remember to publish them!');
}
